...
DBOW_Model_Path = os.path.abspath(os.path.join(
    backend_directory, "course_studyprogram_recommend", "doc2vec_dbow_dm_wiki_article_200_dim", "doc2vec_dbow.model"))
...
def KeywordEmbedding(): 
    ...
    for node in keyword_nodes:
        ...
        if node['abstract'] is not None:
            ...
            cql = """MATCH (n:keyword)
                WHERE n.name='{name}'
                set n.embedding='{embedding}'""".format(name=keyword_info['name'].replace("'", ""), embedding=keyword_info['embedding'])
            try:
                graph.run(cql)
            except Exception as e:
                print(e)
        ...
        if node['abstract'] is None:
            ...
            print("keyword Node {count}: {n} failed, has no abstract".format(count=count, n=node['name']))
    print('{} keyword nodes have embedding'.format(
        len(keyword_list)))  
...
def RelevantKeywordEmbedding():  
    ...
    count=0
    for node in relevant_keywords_nodes:
        count=count+1
        if node['abstract'] is not None:
            relevant_keywords_info['name']=node['name']
            print("relevant_keywords Node " + str(count)+ ": " + relevant_keywords_info['name'])
            relevant_keywords_info['abstract']=node['abstract']
            relevant_keywords_info['embedding']=doc2vecEmbedding(node['abstract']).tolist()
            # print(relevant_keywords_info['embedding'])
            relevant_keywords_list.append(relevant_keywords_info)  
            cql = """MATCH (n:relevant_keywords)
                    WHERE n.name='{name}'
                    set n.embedding='{embedding}'""".format(name=relevant_keywords_info['name'].replace("'", ""), embedding=relevant_keywords_info['embedding'])
            try:
                graph.run(cql)
            except Exception as e:
                print(e)
        ...
        if node['abstract'] is None:
            ...
            print("relevant_keywords Node {count}: {n} failed, has no abstract".format(count=count, n=node['name']))
    print('{} relevant_keywords nodes have embedding'.format(
        len(relevant_keywords_list)))  
...
def CategoryEmbedding():  
    ...
    for node in category_nodes: 
        ...
        cql = """MATCH p=()-[r:is_in]->(m) 
            where m.name = '{name}'
            RETURN p""".format(name=node['name'])
        ...
        if(i>0):
            ...
            for j in range(i):
                ...
                if relationship[j][0].start_node['embedding'] is not None:
                    emb_list = eval('(' + relationship[j][0].start_node['embedding'] + ')')
                    dict['rel_key_embs'][relationship[j][0].start_node['name']] = relationship[j][0].start_node['embedding']
                    vector = vector + np.array(emb_list)
                if relationship[j][0].start_node['embedding'] is None:
                    print("keyword Node {n} has no embedding".format(n=relationship[j][0].start_node['name']))
            ...
            cql = """MATCH (n:category)
                    WHERE n.name='{name}'
                    set n.embedding='{embedding}'""".format(name=dict['name'].replace("'", ""), embedding=dict['cate_embedding'])
            try:
                graph.run(cql)
        ...
        if(i==0):
            ...
            print("category Node {count}: {n} failed, has no relationship".format(count=count, n=node['name']))
    print('{} category nodes have embedding'.format(
        len(category_list)))  
...
def LectureEmbedding():
    ...
    for node in lecture_nodes: 
        ...
        cql = """MATCH p=(n)-[r:has_key]->() 
            where n.name = '{name}'
            RETURN p,r""".format(name=node['name'])
        relationship = list(graph.run(cql))
        i=len(relationship)
        if(i>0):
            dict = {}
            ...
            for j in range(i):
                if relationship[j][0].end_node['embedding'] is not None:
                    dict['rel_key_embs'][relationship[j][0].end_node['name']] = relationship[j][0].end_node['embedding']#方便输出看json结果
                    emb_list = eval('(' + relationship[j][0].end_node['embedding'] + ')')
                    w = float(relationship[j][1].get('weight'))
                    weights = weights + w
                    vector = vector + np.array(emb_list) * w
                if relationship[j][0].end_node['embedding'] is None:
                    print("keyword Node {n} has no embedding".format(n=relationship[j][0].end_node['name']))
            ...
            cql = """MATCH (n:lecture)
                    WHERE n.name='{name}'
                    set n.embedding='{embedding}'""".format(name=dict['name'].replace("'", ""), embedding=dict['lec_embedding'])
            try:
                graph.run(cql)
        ...
        if(i==0):
            ...
            print("lecture Node {count}: {n} failed, has no relationship".format(count=count, n=node['name']))
    print('{} lecture nodes have embedding'.format(
        len(lecture_list)))  
...
def StudyProgramEmbedding():  
    ...
    count=0
    for node in study_program_nodes: 
        count=count+1 
        # print(node['name'])
        ...
        if(i>0):
            ...
            for j in range(i):
                # print(relationship[j][0])
                if relationship[j][0].start_node['embedding'] is not None:
                    emb_list = eval(('(' + relationship[j][0].start_node['embedding'] + ')'),nan_default)
                    dict['rel_key_embs'][relationship[j][0].start_node['name']] = relationship[j][0].start_node['embedding']
                    vector = vector + np.array(emb_list)
                if relationship[j][0].start_node['embedding'] is None:
                    print("keyword Node {n} has no embedding".format(n=relationship[j][0].start_node['name']))
            ...
            cql = """MATCH (n:study_program)
                    WHERE n.name='{name}'
                    set n.embedding='{embedding}'""".format(name=dict['name'].replace("'", ""), embedding=dict['stu_prog_embedding'])
            try:
                graph.run(cql)
        ...
        if(i==0):
            ...
            print("study program Node {count}: {n} failed, has no relationship".format(count=count, n=node['name']))
    print('{} study program nodes have embedding'.format(
        len(study_program_list)))  

def doc2vecEmbedding(text):
    stopwords_list=list(gensim.parsing.preprocessing.STOPWORDS)
    p=[word for word in gensim.utils.simple_preprocess(text) if word not in stopwords_list]
