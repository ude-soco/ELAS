...
Filtered_Keywords_DATA = os.path.abspath(
    os.path.join(
        backend_directory,
        "course_studyprogram_recommend",
        "entities_and_relationships",
        "wiki_filterd_keywords.json",
    )
)
Category_DATA = os.path.abspath(
    os.path.join(
        backend_directory,
        "course_studyprogram_recommend",
        "entities_and_relationships",
        "wiki_entry_categories_summary.json",
    )
)
Links_DATA = os.path.abspath(
    os.path.join(
        backend_directory,
        "course_studyprogram_recommend",
        "entities_and_relationships",
        "sorted_links_count.json",
    )
)
...
OUTPUT_LINK_ENTITIES = os.path.abspath(os.path.join(OUTPUT_DATA, "links.json"))
OUTPUT_HAS_LINK_RELATIONSHIP = os.path.abspath(
    os.path.join(OUTPUT_DATA, "keyword_has_links.json")
)
OUTPUT_CATEGORY_ENTITIES = os.path.abspath(os.path.join(OUTPUT_DATA, "categories.json"))
...
class UpdateNeo4j(object):
    ...
    def access_keywords(self):
        ...
        print("{} keyword nodes got from Neo4j".format(len(self.keyword_list)))

    def delete_nodes(
        self, entity, entity_type_name
    ):  # delete one entity and its relationships
    ...
    def merge(self, title, keywords):
        ...
        for name in keywords:
            cql = """MATCH p=()-[r:has_key]->(m:keyword) where m.name='{tail_name}'
                    RETURN p,r.weight""".format(
                tail_name=name.replace("'", "")
            )
            ...
            if i > 0:  # make sure node exist
                for j in range(i):
                    data[name].append(
                        {has_key[j][0].start_node["name"]: has_key[j][1]}
                    )  # lecture
    ...
    def run(self):
        ...
        with open(Links_DATA, "r", encoding="utf8") as link_file:
            ...
            self.write_weighted_edges(
                self.keyword_has_link, "keyword", "relevant_keywords"
            )
            ...
            with io.open(
                OUTPUT_HAS_LINK_RELATIONSHIP, "w", encoding="UTF8"
            ) as output_file:
        ...
        with open(Category_DATA, "r", encoding="utf8") as cat_file:
            ...
            for dic in category_data:
                ...
                if "abstract" in dic:
                    cql = """MATCH (n:keyword)
                        WHERE n.name='{name}'
                        set n.abstract='{abstract}'""".format(
                        name=keyword_name.replace("'", ""),
                        abstract=dic["abstract"].replace("'", "").replace("\n", ""),
                    )
                    try:
                        self.graph.run(cql)
                    except Exception as e:
                ...
                if "categories" in dic:
                    for cate in dic["categories"]:
                        ...
                        self.keyword_is_in_category.append(
                            [keyword_name, "is_in", cate]
                        )
            ...
            with io.open(
                OUTPUT_IS_IN_RELATIONSHIP, "w", encoding="UTF8"
            ) as output_file:
...
if __name__ == "__main__":
    ...
    # process = Process(target=updateDB.run,)
    # process.start()
    updateDB.run()
