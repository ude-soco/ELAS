...
class LectureExtractor(object):
    ...
    def create_relationships(self):  # create all relationships
        ...
        self.write_edges(
            self.lecture_belongs_to_studyprograms, "lecture", "study_program"
        )
    ...
    def set_attributes_lectures(self):  # set attributes of lecture entity
        t = threading.Thread(
            target=self.set_attributes, args=(self.lecture_infos, "lecture")
        )
        t.setDaemon(False)
        t.start()
    ...
    def set_attributes_study_programs(self):  # set attributes of study program entity
        t = threading.Thread(
            target=self.set_attributes, args=(self.study_program_infos, "study_program")
        )
        t.setDaemon(False)
        t.start()
    ...
    def export_entities_relationships(self):
        ...
        self.export_data(self.lecture_has_key, "/lecture_has_key.json")
        self.export_data(
            self.lecture_belongs_to_studyprograms,
