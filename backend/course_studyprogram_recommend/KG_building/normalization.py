    ...
    def run(self):
        ...
        for node in lecture_nodes:
            ...
            if (i>0):
                ...
                for j in range(i):
                    dict["validate_keywords"][relationship[j].end_node['name']] = float(relationship[j]['weight'])
                ...
                for k,v in normal_keys['validate_keywords'].items():
                        cql = """MATCH p=(n)-[r:has_key]->(m) 
                            ...
                            SET r.weight = '{weight}' """.format(head=dict['name'].replace("'", ""),tail=k.replace("'", ""), weight=v)
                        try:
                            self.graph.run(cql)
                            i=i+1
        ...
        for node in lecture_nodes:
            ...
            if (i>0):
                ...
                dict["validate_links"] = {}
                for j in range(i):
                    dict["validate_links"][relationship[j].end_node['name']] = float(relationship[j]['weight'])
